...
app.post('/module/:moduleId/complete', async (req, res) => {
    try {
        ...
        // Bir sonraki modülün ilk dersini bul
        const nextLessonQuery = `
            SELECT l.id
            FROM modules m1
            JOIN modules m2 ON m1.pathway_id = m2.pathway_id AND m2.id > m1.id
            JOIN lessons l ON l.module_id = m2.id
            WHERE m1.id = $1
            ORDER BY m2.id, l.order_number
            LIMIT 1
        `;
        const nextLessonResult = await pool.query(nextLessonQuery, [moduleId]);

        let redirectUrl = '/dashboard';
        if (nextLessonResult.rows.length > 0) {
            redirectUrl = `/lesson/${nextLessonResult.rows[0].id}`;
        }
